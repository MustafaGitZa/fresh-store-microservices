    ...
    <div class="products-grid">
        <c:forEach var="product" items="${products}">
            <div class="product-card">
                ...
                <form action="/order" method="post" class="product-actions"
                      onsubmit="return confirmOrder('${product.productName}',
                      document.getElementById('qty_${product.productId}').value,
                      document.getElementById('total_${product.productId}').value)">
                    <input type="hidden" name="productName" value="${product.productName}"/>
                    <input type="hidden" name="totalPrice"
                           id="total_${product.productId}" value="${product.price}"/>
                    <input type="number" name="quantity" id="qty_${product.productId}"
                           class="quantity-input" value="1" min="1"
                           max="${product.quantity}"
                           onchange="updateTotalPrice(${product.price},
                           'qty_${product.productId}',
                           'total_${product.productId}')"/>
                    <button type="submit" class="btn btn-success">
                        <img src="https://cdn-icons-png.flaticon.com/512/3144/3144456.png" width="16"/>
                        Order
                    </button>
                </form>
            </div>
